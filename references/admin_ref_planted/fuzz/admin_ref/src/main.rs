...
impl AdminrefAccessFixture {
    pub fn setup() -> Self {
        ...
        ctx.add_program(&program_id, "../../target/deploy/admin_ref.so")
            .unwrap();
        ...
        // withdraw is observable as a state change (not just a no-op).
    ...
    /// Attacker arm — probes `withdraw` with a freshly-minted
...
//
// If the program ever accepted a `withdraw` call signed by anyone
...
fn invariant_withdraw_rejects_unauthorized(fixture: &mut AdminrefAccessFixture) {
    fuzz_assert_eq!(
        ...
        "unauthorized withdraw succeeded on vault {}",
        fixture.vault_pda
    );
}
