// vault_ref — clean variant.
//
// Minimal single-depositor Anchor vault. `vault.amount` tracks deposited
// lamports; `deposit` and `withdraw` move lamports via system transfer +
// direct lamport mutation and keep `vault.amount` in lock-step with the
// real PDA balance net of rent. Used by cf-invariants-anchor Phase 0 as
// the clean side of a balance-conservation reference pair.
